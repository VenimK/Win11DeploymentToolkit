...
try {
    ...
    try {
        ...
        Write-Host "CPU Generation: $gen" -ForegroundColor White
        ...
        if ($cpuName -match "Intel.*Xeon") {
            ...
            Write-Host "Generation: $gen" -ForegroundColor White
        } else {
    ...
    Write-Host "Minimum Required: 4 GB" -ForegroundColor Cyan
...
# Start logging
$logFile = Join-Path -Path (Split-Path -Parent (Split-Path -Parent $MyInvocation.MyCommand.Path)) -ChildPath "Update_Extraction_Log.txt"
Start-Transcript -Path $logFile -Force
...
try {
    ...
    try {
        ...
        Write-Host "Cores: $cpuCores physical, $cpuLogical logical" -ForegroundColor Cyan
        Write-Host "Speed: $cpuSpeed GHz" -ForegroundColor Cyan
        ...
        if ($cpuName -match "Intel.*Xeon") {
            Write-Host "CPU Type: Intel Xeon" -ForegroundColor Cyan
            Write-Host "Check the Windows 11 compatibility list for this specific Xeon model" -ForegroundColor Yellow
        } elseif ($cpuName -match "AMD.*Ryzen") {
            Write-Host "CPU Type: AMD Ryzen" -ForegroundColor Cyan
            Write-Host "Generation: $gen" -ForegroundColor Cyan
        } else {
            Write-Host "CPU Type: Other" -ForegroundColor Cyan
            Write-Host "Check the Windows 11 compatibility list for this specific CPU model" -ForegroundColor Yellow
        }
    } 
    catch {
...
System meets Windows 11 requirements." -ForegroundColor Green
    ...
    # Find the ISO file
    $isoFolder = Join-Path -Path (Split-Path -Parent (Split-Path -Parent $MyInvocation.MyCommand.Path)) -ChildPath "Windows11"
